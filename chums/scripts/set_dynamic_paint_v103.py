...
canvases = ['env.loonbeach:beach_canvas_010','env.loonbeach:beach_canvas_011',
            'env.loonbeach:beach_canvas_012','env.loonbeach:beach_canvas_013',
            'env.loonbeach:beach_canvas_014','env.loonbeach:beach_canvas_015']
...
shots = {'010':10,'020':10,'030':10,'040':10,'050':10}
#103_sc08
...
#process shot alembic offsets to drive render range
...
#setup CANVASes
for this_canvas in canvases:
    canvas = bpy.data.objects[this_canvas]
    ...
    this_version = ''
    canvas_id = this_canvas[-2:]
    map_id = ("map" + canvas_id)
    ...
    dp_canvas_mod.canvas_settings.canvas_surfaces[0].uv_layer = map_id
    dp_canvas_mod.canvas_settings.canvas_surfaces[0].use_output_a = True
    dp_canvas_mod.canvas_settings.canvas_surfaces[0].output_name_a = (cache.name.replace('_mayaCache','')[:-6] + '_' + this_pass + canvas_id + '_')
