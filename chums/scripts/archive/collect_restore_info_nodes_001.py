...
def gn_gather_deps(gntree):
    ...
    for node in gntree.nodes:
        ...
        if node.type in ['OBJECT_INFO','COLLECTION_INFO','STORE_NAMED_ATTRIBUTE','INPUT_ATTRIBUTE']:
        ...
        if node.type in ['OBJECT_INFO','COLLECTION_INFO','STORE_NAMED_ATTRIBUTE','INPUT_ATTRIBUTE']:
            ...
            if node.type == 'OBJECT_INFO':
                try:
                    ...
                    print("Failed to set:", node.name, node.inputs[0].identifier, "to:", node.label)
            elif node.type == 'COLLECTION_INFO':
                try:
                    node.inputs[0].default_value = bpy.data.collections[node.label]
                except:
                    print("Failed to set:", node.name, "to:", node.label)
...
for ob in bpy.context.selected_objects:
    for mod in ob.modifiers:
        if mod.type == 'NODES':
            ...
            my_restore = gn_restore_deps(mod.node_group)
