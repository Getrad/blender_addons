...
if '0053_' in theprojectroot:
    ...
    base_path = os.path.join(theprojectroot, 'asset\\library\\node_groups')
    empty_blend = os.path.join(theprojectroot, 'asset\\library\\node_groups\\empty.blend')
...
def register():
    ...
    bpy.utils.register_class(GetNodeName)
    bpy.utils.register_class(ExportNodes)
    bpy.utils.register_class(ImportNodes)
    bpy.utils.register_class(DeleteNodes)
    bpy.utils.register_class(CUSTOMNODES_UL_customnodes)
    bpy.utils.register_class(CustomNodeGroups)
    bpy.utils.register_class(CustomNodeProperties)
    bpy.utils.register_class(CUSTOMNODES_PT_NodeImporter)
    bpy.utils.register_class(CUSTOMNODES_PT_NodeExporter)    
    bpy.types.Scene.custom_nodes = bpy.props.PointerProperty(type=CustomNodeProperties)

def unregister():
    #bpy.utils.unregister_module(__name__)
    bpy.utils.unregister_class(GetNodeName)
    bpy.utils.unregister_class(ExportNodes)
    bpy.utils.unregister_class(ImportNodes)
    bpy.utils.unregister_class(DeleteNodes)
    bpy.utils.unregister_class(CUSTOMNODES_UL_customnodes)
    bpy.utils.unregister_class(CustomNodeGroups)
    bpy.utils.unregister_class(CustomNodeProperties)
    bpy.utils.unregister_class(CUSTOMNODES_PT_NodeImporter)
    bpy.utils.unregister_class(CUSTOMNODES_PT_NodeExporter)  
